...
(function () {
  ...
  function maleSlf(z, s) {
    ...
    const mitZeit = s.phase === 'schreiben' || s.phase === 'pruefen' || (s.phase === 'tafel' && s.pruefung === 'kinder');
    sTimer.setze(s.restMs, s.phasenDauer, z.raum.pause, mitZeit);
    $('#s-timer').hidden = !mitZeit;

    const schreiben = s.phase === 'schreiben' || s.phase === 'einsammeln';
    $('#s-schreiben').hidden = !schreiben;
    $('#s-auswertung').hidden = s.phase !== 'auswertung';
    $('#s-pruefen').hidden = s.phase !== 'pruefen';
    ...
    if (s.phase === 'pruefen' && s.pruefliste) {
      karteHerein($('#s-pruefen'), schluessel + '|pruefen');
      const gesperrt = s.ichGeprueft || z.raum.pause;
      setzeHtml($('#s-pruefliste'), s.pruefliste.map((kat) => {
        const zeilen = kat.eintraege.map((e) => `
          <li class="pruefeintrag ${e.dagegen ? 'dagegen' : ''} ${e.eigen ? 'eigen' : ''}">
            <span class="was">${esc(e.text)}${e.anzahl > 1 ? ` <small>${e.anzahl}×</small>` : ''}</span>
            ${e.eigen ? '<span class="stimmen">deine Antwort</span>'
              : `<button type="button" class="nein-knopf" data-kat="${kat.kat}" data-schluessel="${esc(e.schluessel)}" aria-pressed="${e.dagegen}" ${gesperrt ? 'disabled' : ''}>${e.dagegen ? '✗ stimmt nicht' : 'stimmt nicht?'}</button>`}
          </li>`).join('') + kat.falsch.map((t) => `
          <li class="pruefeintrag automatisch"><span class="was">${esc(t)}</span><span class="stimmen">falscher Anfangsbuchstabe</span></li>`).join('');
        return `<section class="pruefkarte"><h3>${esc(kat.kategorie)}</h3><ul>${zeilen || '<li class="pruefeintrag automatisch"><span class="stimmen">keine Antworten</span></li>'}</ul></section>`;
      }).join(''));
      $('#s-geprueft').hidden = s.ichGeprueft;
      $('#s-geprueft').disabled = z.raum.pause;
      $('#s-geprueft-text').textContent = s.ichGeprueft ? 'Danke! Warte auf die anderen.' : '';
    }

    if (s.phase === 'tafel' && s.tafel) {
      karteHerein($('#s-tafel').firstElementChild, schluessel + '|tafel');
      $('#s-tafel-text').textContent = s.pruefung === 'kinder' ? 'Punkte aus dieser Runde. Die nächste Runde startet gleich von selbst.' : 'Punkte aus dieser Runde. Gleich geht es weiter.';
      setzeHtml($('#s-tafelliste'), ranglisteHtml(s.tafel, z.ich.id, z.raum.wertung === 'teams', true));
      const meine = (s.ergebnis || []).map((kat) => ({ kategorie: kat.kategorie, e: kat.eintraege.find((x) => x.id === z.ich.id) })).filter((x) => x.e);
      $('#s-meine-block').hidden = !meine.length;
      const grundText = (e) => ({
        leer: 'leer',
        buchstabe: 'falscher Anfangsbuchstabe',
        abgelehnt: e.stimmen ? `rausgeflogen, ${e.stimmen.dagegen} von ${e.stimmen.pruefer} dagegen` : 'rausgeflogen',
        lehrkraft: 'ungültig',
      }[e.grund] || '');
      setzeHtml($('#s-meine'), meine.map(({ kategorie, e }) => `
        <li class="${e.gueltig ? '' : 'ungueltig'}">
          <span class="kat">${esc(kategorie)}</span>
          <span class="was">${e.text ? esc(e.text) : '–'}${e.grund ? ` <small>${esc(grundText(e))}</small>` : ''}</span>
          <span class="p">${e.punkte}</span>
        </li>`).join(''));
    }
  }
  ...
  $('#s-pruefliste').addEventListener('click', (e) => {
    const b = e.target.closest('.nein-knopf');
    if (!b || b.disabled || !zustand || zustand.raum.pause) return;
    const wert = b.getAttribute('aria-pressed') !== 'true';
    socket.emit('kind:spiel', { art: 'dagegen', kat: Number(b.dataset.kat), schluessel: b.dataset.schluessel, wert }, (a) => {
      if (a && !a.ok && a.fehler) G.melde(a.fehler, 'fehler');
    });
  });
  $('#s-geprueft').addEventListener('click', () => {
    socket.emit('kind:spiel', { art: 'geprueft' });
  });
