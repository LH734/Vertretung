...
(function () {
  ...
  // Aus dem Gruppenmodus geöffnet: /lehrer#raum=ABCD&schluessel=…
  if (!passiv && location.hash.includes('schluessel=')) {
    const h = new URLSearchParams(location.hash.slice(1));
    if (h.get('raum') && h.get('schluessel')) {
      try { sessionStorage.setItem(SPEICHER, JSON.stringify({ code: h.get('raum'), schluessel: h.get('schluessel') })); } catch (e) { /* ohne Speicher */ }
    }
  ...
  function maleSlf(z, s, teams) {
    ...
    bsTimer.setze(s.restMs, s.phasenDauer, z.raum.pause, mitZeit);
    $('#bs-timer').hidden = !mitZeit;
    const hinweise = {
      ...
      pruefen: 'Die Kinder prüfen gegenseitig.',
    ...
    $('#bs-pruefen').hidden = s.phase !== 'pruefen';

    if (s.phase === 'pruefen') {
      karteHerein($('#bs-pruefen'), 'pruefen' + s.runde);
      const online = z.spieler.filter((p) => p.online);
      const fertig = new Set(s.pruefFertigIds || []);
      $('#bs-pruef-fortschritt').textContent = `${online.filter((p) => fertig.has(p.id)).length} von ${online.length} haben fertig geprüft`;
      setzeHtml($('#bs-pruef-fertige'), online.slice().sort((a, b) => a.name.localeCompare(b.name, 'de')).map((p) =>
    ...
    if (s.phase === 'tafel' && s.tafel) {
      ...
      const raus = [];
      for (const kat of s.ergebnis || []) {
        for (const e of kat.eintraege) if (e.grund === 'abgelehnt') raus.push({ kat: kat.kategorie, e });
      }
      $('#bs-raus-block').hidden = !raus.length;
      setzeHtml($('#bs-raus'), raus.slice(0, 8).map(({ kat, e }) =>
        `<li><span class="was">${esc(e.text)}</span> <span class="wer">${esc(kat)} · ${esc(e.name)}${e.stimmen ? ` · ${e.stimmen.dagegen} von ${e.stimmen.pruefer} dagegen` : ''}</span></li>`).join('') +
        (raus.length > 8 ? `<li><span class="wer">und ${raus.length - 8} weitere</span></li>` : ''));
    }
  }
  ...
  const einstellungen = new window.Einstellungen($('#r-einstellungen'), schicke);
  ...
  function maleRegie(z) {
    ...
    ueber.textContent = !s ? 'Runde überspringen'
      : s.art === 'slf' ? ({ schreiben: 'Schreibzeit beenden', einsammeln: 'Zur Auswertung', pruefen: 'Prüfrunde beenden', auswertung: 'Auswertung abschließen', tafel: 'Nächste Runde' }[s.phase] || 'Weiter')
  ...
  $('#r-start').addEventListener('click', () => {
    ...
    socket.emit('lehrer:start', einstellungen.texte(), (a) => {
      if (a && !a.ok) G.melde(a.fehler, 'fehler');
    });
  });
